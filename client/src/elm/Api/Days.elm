...
dayDecoder = Dec.object5 Day
    ("id" := Dec.string)
    ("title" := Dec.string)
    ("description" := Dec.map descriptionStringToParts Dec.string)
    ("created" := Dec.int)
    ("modified" := Dec.int)

-- day descriptions are read as a list of DayParts, where the part is either
-- some markdown text, or a list of entry IDs
type DescriptionPart = MarkdownPart String | EntriesPart (List String)
...
descriptionPartsToString parts =
  let
    fn = \part str -> case part of
        MarkdownPart s -> str ++ s
        EntriesPart es -> str ++ "@entries" ++ List.foldl (\e s -> s ++ ":" ++ e) "" es
  in
    List.foldl fn "" parts

descriptionStringToParts : String -> List DescriptionPart
descriptionStringToParts str = case Parser.parse partParser str of
    Ok res -> res
    Err _ -> []
...
descriptionPartsToEntryIds parts =
  let
    fn = \part out -> case part of
      EntriesPart es -> out ++ es
      _ -> out
  in
    List.foldl fn [] parts
...
partParser =
  let
    entries =
      let
        toString = \charList -> List.foldr (\c s -> String.cons c s) "" charList
        sep = Parser.symbol ':'
        entryId = Parser.some (Parser.satisfy (\c -> Char.isUpper c || Char.isLower c || Char.isDigit c)) |> Parser.map toString
        entryIds = Parser.separatedBy entryId sep
        hasEntries = Parser.map EntriesPart (sep `Parser.andThen` \_ -> entryIds)
      in
        Parser.token "@entries" `Parser.andThen` \_ -> Parser.optional hasEntries (EntriesPart [])
    loop str bits =
      let
        next = \s bit -> Parser.recursively (\() -> loop s bit)
        foundEntries = entries `Parser.andThen` (\es -> next "" (bits ++ [MarkdownPart str, es]))
        finishedSoReturnMarkdown = Parser.end |> Parser.map (\_ -> bits ++ [MarkdownPart str])
        takeCharAndLoop = Parser.satisfy (always True) `Parser.andThen` (\c -> next (str ++ String.fromChar c) bits)
      in
...
addDayEncoder day =
  let
    entryIds = descriptionPartsToEntryIds day.description
    description = descriptionPartsToString day.description
  in
    Enc.object
