...
doRemoveDay dayId =
  Task.perform (always DoneRemove) (always DoneRemove) (Days.remove dayId)
...
editModal = Modals.renderWith (\model ->
    ...
    , onClose = Close
    , mdl = Mdl
    , cover = if model.removing then [ removeModal model ] else []
...
dayModalHtml isEditMode model =
  let
    errorString = case model.error of
        Just DayBadTitle -> "Day title required"
        Just DayBadDescription -> "Day description required"
        _ -> "Something eery happened!"
  in
    div [ class "day-modal" ]
        [ table [ class "inputs" ]
            [ inputRow "Title" <|
        ...
        , div [ class "bottom-row" ]
            [ Button.render Mdl [5] model.mdl
                [ Button.raised
                , Button.colored
                , Button.disabled `when` (model.title == "" || model.description == "")
                , Button.onClick (if isEditMode then DoEditDay else DoAddDay)
                , cs "add-day-button"
                ]
                [ text (if isEditMode then "Save Changes" else "Add Day") ]
            , if isJust model.error
            ...
            , isEditMode ?
                div [ class "delete" ]
                    [ Button.render Mdl [6] model.mdl
                        [ Button.icon
                        , Button.ripple
                        , Button.onClick ShowRemoveModal
                        ]
                        [ Icon.i "delete"]
                    ]
            ]
