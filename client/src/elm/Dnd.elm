...
type Model listId uniqueId = Model (TheModel listId uniqueId)
...
type alias DragPosition listId uniqueId = (listId, Position uniqueId, Position uniqueId)
...
update msg (Model model) = case msg of
    ...
    DragComplete ->
      let
        act = case (model.listId, model.selectedId, model.dragPosition) of
            (Just listId, Just id, Just pos) -> Just (MovedTo (listId,id) pos)
            _ -> Nothing
      in
...
type alias Options listId = { listId: listId, enabled: Bool }
  ...
  let
        ...
        el =
            div [ class ("dnd-item" ++ if isBeingDragged && model.dragInProgress then " being-dragged" else "")
                ...
                , div [ class "dnd-item-before"
                    , onDragOver (if isDisabled || isBeingDragged || isBelowDragged then Nothing else Just (listId, lastPos, AtId id))
                    ] []
                , div [ class "dnd-item-after"
                    , onDragOver (if isDisabled || isBeingDragged || isAboveDragged then Nothing else Just (listId, AtId id, nextPos))
                    ] []
                ]
      in
        (toString id, el)
    ...
    toDndSpacer ((listId, a, b) as dragPosition) =
      let
        ...
        isOnlySpacer = a == AtBeginning && b == AtEnd
        el =
            div [ class
                    (  "dnd-spacer"
                    ++ (if model.dragPosition == Just dragPosition && model.dragInProgress && options.enabled then " active" else "")
                    ++ (if isNearDragged then " near-dragged" else "")
                    ++ (if isAboveDragged then " above" else "")
                    ++ (if isBelowDragged then " below" else "")
                    ++ (if isOnlySpacer then " singleton" else "")
                    )
                , onDragOver (if options.enabled then Just dragPosition else Nothing)
                ] []
      in
      ...
      let
        fn (idx,(itemId, itemHtml)) output =
          let
            ...
            item1 rest = if lastPos == AtBeginning then toDndSpacer (listId, AtBeginning, AtId itemId) :: rest else rest
            item2 rest = toDndItem lastPos (itemId, itemHtml) nextPos :: rest
            item3 rest = toDndSpacer (listId, AtId itemId, nextPos) :: rest
          in
            item1 <| item2 <| item3 <| output
      in
        case Array.isEmpty itemArr of
            False -> Array.foldr fn [] (Array.indexedMap (\i a -> (i,a)) itemArr)
            True  -> [toDndSpacer (listId, AtBeginning, AtEnd)]
  in
    -- our output is a dnd-items div wrapping spacer-separated draggable items:
    Keyed.node "div"
        [ class ("dnd-items" ++ if model.dragInProgress && options.enabled then " active" else "")
