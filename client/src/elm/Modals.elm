...
renderWith :
    (WithMdl childModel -> RenderOptions childMsg)
    -> (parentModel -> WithMdl childModel)
    -> (childMsg -> parentMsg)
    -> (parentModel -> Html parentMsg)
renderWith modal toModel fromMsg =
    \parentModel ->
        let
            thisModel =
                toModel parentModel
        in
            render (modal thisModel) thisModel |> Html.map fromMsg


type alias WithMdl model =
            ...
            [ div [ class "title" ]
                ...
                , not hideClose
                    ? Button.render mdl
                        ...
                        , when preventClose Button.disabled
                        ]
                        [ Icon.i "close" ]
                ]
            , div [ class "content" ]
...
choiceToRenderOptions opts model =
    ...
    , content =
        div [ class "choice-modal" ]
            ...
            , div [ class "buttons" ]
                [ not opts.hideCancel
                    ? Button.render opts.mdl
                        [ 200, 0 ]
                        model.mdl
                        [ Button.raised
                        , Button.colored
                        , Options.onClick opts.onCancel
                        , cs "cancel-button"
                        ]
                ...
                , not opts.hidePerform
                    ? Button.render opts.mdl
                        ...
                        [ Button.raised
                        , Button.colored
                        , Options.onClick opts.onPerform
                        , cs "perform-button"
                        ]
...
choice opts model =
    render (choiceToRenderOptions opts model) model
